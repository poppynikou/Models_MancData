...
for patient in patients:
    ...
    postprocessing_directory = base_path + '/HN_'+str(patient)+'/CBCT_GROUPWISE/postprocessing/'
    if not os.path.exists(postprocessing_directory):
        os.mkdir(postprocessing_directory)

    for CBCT in CBCTs:

        CBCT_original =  base_path + '/HN_'+str(patient)+'/CBCT_GROUPWISE/preprocessing/CBCT_' + str(CBCT) + '.nii.gz'
        ...
        for itteration in np.arange(1,no_iterations+1):
            ...
            affine = base_path + '/HN_'+str(patient)+'/CBCT_GROUPWISE/affine_'+str(itteration)+'/comp_affine_' + str(CBCT) + '.txt'
