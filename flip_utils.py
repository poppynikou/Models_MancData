...
def create_rotation_affine(axis, angle):
...
def resample_binary(ref_img, float_img, transformation, resampled_img):
    
    reg_resample = 'T:/Poppy/niftireg_executables/reg_resample.exe'

    command = reg_resample + ' -ref ' + ref_img + ' -flo ' + float_img + ' -trans ' + transformation + ' -res ' + resampled_img + ' -inter 1 -pad 0 -omp 12'
    os.system(command)